package cz.muni.fi.editor.cmisserver.types.vendor;
...
// copied from chemistry-opencmis-client implementation
...
public final class TypeUtils {
  ...
  public static void writeToXML(
      TypeDefinition type, OutputStream stream) throws XMLStreamException {
    if (type == null) {
  ...
  public static TypeDefinition readFromJSON(InputStream stream)
  ...
  public static boolean checkQueryName(String queryName) {
    return queryName != null && queryName.length() > 0 && queryName.indexOf(' ') < 0
        && queryName.indexOf('\t') < 0
  ...
  public static List<ValidationError> validateTypeDefinition(TypeDefinition type) {
    ...
    } else if (type.getBaseTypeId() == BaseTypeId.CMIS_SECONDARY && Boolean.TRUE
        .equals(type.isControllableAcl())) {
      errors.add(
          new ValidationError("controllableACL",
              "ControllableACL flag must be FALSE for secondary types."));
    }
    ...
    if (type.isIncludedInSupertypeQuery() == null) {
      errors.add(new ValidationError("includedInSupertypeQuery",
          "IncludedInSupertypeQuery flag must be set."));
    }
    ...
    if (type instanceof DocumentTypeDefinition) {
      ...
      if (docType.getContentStreamAllowed() == null) {
        errors.add(
            new ValidationError("contentStreamAllowed", "ContentStreamAllowed flag must be set."));
      }
  ...
  public static List<ValidationError> validatePropertyDefinition(PropertyDefinition<?> propDef) {
    ...
    if (propDef.isOrderable() == null) {
      ...
      if (propDef.getCardinality() == Cardinality.MULTI) {
        errors.add(
            new ValidationError("orderable",
                "Orderable flag is set to TRUE for a multi-value property."));
      }
    }
    ...
    if (propDef instanceof PropertyIdDefinition) {
      if (propDef.getPropertyType() != PropertyType.ID) {
        errors.add(
            new ValidationError("propertyType",
                "Property type does not match the property definition."));
      }
